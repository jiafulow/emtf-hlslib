...
#pragma HLS DATA_PACK variable=quad_param_table
  ...
  LOOP_I: for (unsigned i = 0; i < (M_TABLE * N_TABLE); i++) {

#pragma HLS UNROLL

    const quad_param_t x_i = {
      static_cast<dio_patt_param_t>(pattern_col_start_table[i]),
...
void trkbuilding_find_ph_median_op(
...
//#pragma HLS INLINE
  ...
  ph_median = (
      (static_cast<emtf_phi_t>(curr_trk_col_corr) << bits_to_shift) + (1u << (bits_to_shift - 1))
  );
...
#pragma HLS INLINE
  ...
  // Loop over segments
  LOOP_PHI_2: for (unsigned i = 0; i < num_site_segments; i++) {
...
#pragma HLS UNROLL
    ...
    const bool valid = (
        (seg_valid_mhph[i] == 1) and \
        (curr_trk_qual_gt_0 == 1) and \
        (seg_zones_mhph[i][bit_sel_zone] == 1) and \
        (seg_tzones_mhph[i][bit_sel_tzone] == 1) and \
...
void trkbuilding_match_ph_pack_op(
    ...
    const T_IN          ph_diff           [trkbuilding_internal_config::num_site_segments],
    const bool_t        ph_diff_v         [trkbuilding_internal_config::num_site_segments],
    const trk_col_t&    curr_trk_col_corr ,
    T_OUT               stage_0_out       [trkbuilding_internal_config::num_gate_segments]
) {
  static_assert(is_same<T_IN, dio_ph_diff_t>::value, "T_IN type check failed");
  static_assert(is_same<T_OUT, trkbuilding_internal_config::stage_0_out_t>::value, "T_OUT type check failed");
...
#pragma HLS DATA_PACK variable=stage_0_out_tmp
  ...
  // Loop over segments
  LOOP_PHI_3: for (unsigned i = 0; i < num_site_segments; i++) {
...
template <typename T_IN, typename T_OUT=void>
void trkbuilding_match_ph_argmin_op(
    const T_IN     in0                     [trkbuilding_internal_config::num_gate_segments],
  ...
  static_assert(is_same<T_IN, trkbuilding_internal_config::stage_0_out_t>::value, "T_IN type check failed");
...
#pragma HLS DATA_PACK variable=stage_0_out
  ...
  LOOP_PHI_1: for (unsigned i = 0; i < num_site_segments; i++) {
...
//#pragma HLS INLINE
  ...
  trkbuilding_match_ph_site_op<m_site_0_tag>(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi[0], feat_emtf_bend[0], feat_emtf_theta1[0], feat_emtf_theta2[0], feat_emtf_qual[0],
      ph_seg[0], ph_seg_v[0]
  );
  trkbuilding_match_ph_site_op<m_site_1_tag>(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi[1], feat_emtf_bend[1], feat_emtf_theta1[1], feat_emtf_theta2[1], feat_emtf_qual[1],
      ph_seg[1], ph_seg_v[1]
  );
  trkbuilding_match_ph_site_op<m_site_2_tag>(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi[2], feat_emtf_bend[2], feat_emtf_theta1[2], feat_emtf_theta2[2], feat_emtf_qual[2],
      ph_seg[2], ph_seg_v[2]
  );
  trkbuilding_match_ph_site_op<m_site_3_tag>(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi[3], feat_emtf_bend[3], feat_emtf_theta1[3], feat_emtf_theta2[3], feat_emtf_qual[3],
      ph_seg[3], ph_seg_v[3]
  );
  trkbuilding_match_ph_site_op<m_site_4_tag>(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi[4], feat_emtf_bend[4], feat_emtf_theta1[4], feat_emtf_theta2[4], feat_emtf_qual[4],
      ph_seg[4], ph_seg_v[4]
  );
  trkbuilding_match_ph_site_op<m_site_5_tag>(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi[5], feat_emtf_bend[5], feat_emtf_theta1[5], feat_emtf_theta2[5], feat_emtf_qual[5],
      ph_seg[5], ph_seg_v[5]
  );
  trkbuilding_match_ph_site_op<m_site_6_tag>(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi[6], feat_emtf_bend[6], feat_emtf_theta1[6], feat_emtf_theta2[6], feat_emtf_qual[6],
      ph_seg[6], ph_seg_v[6]
  );
  trkbuilding_match_ph_site_op<m_site_7_tag>(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi[7], feat_emtf_bend[7], feat_emtf_theta1[7], feat_emtf_theta2[7], feat_emtf_qual[7],
      ph_seg[7], ph_seg_v[7]
  );
  trkbuilding_match_ph_site_op<m_site_8_tag>(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi[8], feat_emtf_bend[8], feat_emtf_theta1[8], feat_emtf_theta2[8], feat_emtf_qual[8],
      ph_seg[8], ph_seg_v[8]
  );
  trkbuilding_match_ph_site_op<m_site_9_tag>(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi[9], feat_emtf_bend[9], feat_emtf_theta1[9], feat_emtf_theta2[9], feat_emtf_qual[9],
      ph_seg[9], ph_seg_v[9]
  );
  trkbuilding_match_ph_site_op<m_site_10_tag>(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi[10], feat_emtf_bend[10], feat_emtf_theta1[10], feat_emtf_theta2[10], feat_emtf_qual[10],
      ph_seg[10], ph_seg_v[10]
  );
  trkbuilding_match_ph_site_op<m_site_11_tag>(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi[11], feat_emtf_bend[11], feat_emtf_theta1[11], feat_emtf_theta2[11], feat_emtf_qual[11],
      ph_seg[11], ph_seg_v[11]
  );
}

// _____________________________________________________________________________
template <typename T_IN, typename T_OUT>
void trkbuilding_find_th_median_of_nine_op(
    const T_IN   in0 [trkbuilding_internal_config::num_th_median_entries],
...
#pragma HLS DATA_PACK variable=ternary_tree
  ...
  // Fetch input
  LOOP_MEDIAN_1: for (unsigned i = 0; i < N; i++) {
...
#pragma HLS UNROLL
  ...
  // Tree reduce
  LOOP_MEDIAN_2: for (int i = ((N - 1) / 2) - 1; i >= 0; i--) {
...
template <typename T=void>
void trkbuilding_find_th_median_op(
    const emtf_theta_t feat_emtf_theta_ambi [num_emtf_sites * 2],
...
#pragma HLS UNROLL
    ...
    emtf_assert(
        (theta_indices_table[i] < (num_emtf_sites * 2)) and \
...
#pragma HLS INLINE
  ...
  const diff_t th_diff_1 = (
      ((th1 != invalid_marker_th) and (th_diff_tmp_1 <= invalid_marker_th_diff)) ?
      static_cast<diff_t>(th_diff_tmp_1) : invalid_marker_th_diff
  );
...
//#pragma HLS INLINE
  ...
  LOOP_THETA_2: for (unsigned i = 0; i < num_emtf_sites; i++) {
...
#pragma HLS ARRAY_RESHAPE variable=th_seg_v complete dim=0
  ...
  trkbuilding_match_ph_op(
      emtf_phi, emtf_bend, emtf_theta1, emtf_theta2, emtf_qual1, seg_zones,
      seg_tzones, seg_valid, curr_trk_qual, curr_trk_patt, curr_trk_col, curr_trk_zone,
      curr_trk_tzone, feat_emtf_phi, feat_emtf_bend, feat_emtf_theta_ambi, feat_emtf_qual, ph_seg,
      ph_seg_v
  );
  ...
  trkbuilding_extract_features_op(
      feat_emtf_phi, feat_emtf_bend, feat_emtf_theta, feat_emtf_qual, ph_seg, th_seg_v,
